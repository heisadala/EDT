...
// https://www.w3schools.com/cssref/css_colors.php
  ...
  const getTable = function (filter, name) {
    ...
    for (i=0; i<entryInfoObjects.length; i++) {
      ...
      if (filter == 'PROJECT') {
        if (entryInfoObjects[i].assignment == name) {
          localTable[j] = Object.assign({}, entryInfoObjects[i]);
          j++;
        }
      }
      if (filter == 'PROJECT_EDT') {
        if (entryInfoObjects[i].assignment.substr(0,3) == name) {
          localTable[j] = Object.assign({}, entryInfoObjects[i]);
          j++;
        }
      }
  ...
  for (p=0; p < projectInfoObjects.length; p++) {
      ...
      if (projectInfoObjects[p].name.substr(0,3) != 'EDT'){
          PROJECT[PROJECT_INDEX] = new Project(projectInfoObjects[p].name, 
                                color_index, 
                                getTable('PROJECT', projectInfoObjects[p].name)
                              );
          PROJECT[PROJECT_INDEX].setIncExp();
          for (c=0; c < categoryInfoObjects.length; c++) {
            PROJECT[PROJECT_INDEX].setCategories(categoryInfoObjects[c].name, c);
          }
          PROJECT_INDEX++;
          color_index++;
      }                 
  ...
  PROJECT[PROJECT_INDEX] = new Project('EDT', 
      6, 
      getTable('PROJECT_EDT', 'EDT')
  );
  PROJECT[PROJECT_INDEX].setIncExp();
  for (c=0; c < categoryInfoObjects.length; c++) {
      PROJECT[PROJECT_INDEX].setCategories(categoryInfoObjects[c].name, c);
  ...
  PROJECT_INDEX++;
  console.log('PROJECT',PROJECT_INDEX, PROJECT);
  ...
  const dataAllBanksIncExpSav = {
    ...
    datasets: [{
      ...
      data: [INCOMES_TOTAL, EXPENSES_TOTAL, BALANCE_TOTAL],
      backgroundColor: [
        CHART_COLORS.credit,
        CHART_COLORS.debit,
  ...
  const project_inc = [];
  i=0;
  let sortedIncProjects = PROJECT.sort((p1, p2) => (p1.incomes < p2.incomes) ? 1 : (p1.incomes > p2.incomes) ? -1 : 0);
  for (p=0; p < PROJECT.length; p++) {
    if (PROJECT[p].getIncomes() > 0 && PROJECT[p].getName() != 'AUTRE' && PROJECT[p].getName() != 'INTERN') {
